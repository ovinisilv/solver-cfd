...
for i in range (100,299,20):
#	print (i)
	cont = cont+1
 	#print (i)

	T   = np.loadtxt('transient/data/T'+str(i)+'.dat', skiprows=0, unpack=True)
...
#	plt.contour(x,y,C,colors=('k',),linestyles=('--',),linewidths=(2,), levels=[1/(S+1)])
	plt.contourf(x,y,T,100, cmap=plt.cm.get_cmap('RdBu_r'))
...
#plt.gca().add_patch(circle3)
	plt.savefig('output/temperature'+str(cont)+'.png', bbox_inches='tight')
...
#	plt.gca().add_patch(circle)
	#plt.gca().add_patch(circle2)
	#plt.gca().add_patch(circle3)
#	plt.savefig('output/streamlines'+str(cont)+'.png', bbox_inches='tight')
	#	plt.show()
