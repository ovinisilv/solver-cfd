...
########### PRESSURE
	...
	plt.colorbar(mpl.cm.ScalarMappable(norm=mpl.colors.Normalize(vmin=Pmin, vmax=Pmax), cmap=plt.cm.get_cmap('RdBu_r')))
	plt.axis('scaled')
	plt.xlabel('x')
	plt.ylabel('y')
...
########### TEMPERATURE
	plt.figure(figsize=(8, 8))
	plt.contour(x,y,T,colors=('k',),linestyles=('--',),linewidths=(2,), levels=[1/(S+1)])
	plt.contourf(x,y,T,50, cmap=plt.cm.get_cmap('RdBu_r'))
	plt.colorbar(mpl.cm.ScalarMappable(norm=mpl.colors.Normalize(vmin=Tmin, vmax=Tmax), cmap=plt.cm.get_cmap('RdBu_r')))
	plt.axis('scaled')
	plt.xlabel('x')
	plt.ylabel('y')
...
#plt.plot(X_stream,Y_stream, 'k-.',linewidth=0.4)
	...
	plt.colorbar(mpl.cm.ScalarMappable(norm=mpl.colors.Normalize(vmin=0, vmax=3), cmap=plt.cm.get_cmap('RdBu_r')))
